...
def experiment_from_img(img, src_config=None, nch=4, ncyc=3):
    ...
    if src_config is not None:
        config_data = dict(src_config._conf)
        config_data.update(shape_config)
    else:
        shape_config.update(dict(
        ...
        config_data = shape_config

    config_data['all_channel_names'] = ['CH-{}-{}'.format(i, j) for i in range(ncyc) for j in range(nch)]
    config = cytokit_config.CytokitConfigV1(config_data)
